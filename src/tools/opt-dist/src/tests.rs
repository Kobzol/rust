...
pub fn run_tests(env: &dyn Environment) -> anyhow::Result<()> {
    ...
    copy_directory(
        &libstd_dir.join("lib/rustlib").join(&host_triple).join("lib"),
        &rustc_dir.join("lib/rustlib").join(&host_triple).join("lib"),
    )?;
    ...
    // works correctly.
    copy_directory(&extracted_src_dir.join("lib/rustlib/src"), &rustc_dir.join("lib/rustlib/src"))?;
